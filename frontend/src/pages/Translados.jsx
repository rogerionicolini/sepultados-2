...
function destinoLabel(row, tumMap) {
  const d = row.destino;
  if (d === "outro_tumulo")
    return `Outro Túmulo: ${row.tumulo_destino?.identificador || tumMap.get(String(row.tumulo_destino)) || "-"}`;
  if (d === "outro_cemiterio") return `Outro Cemitério: ${row.cemiterio_nome || "-"}`;
  return "Ossário";
}

export default function Translados() {
  ...
  async function carregar() {
    try {
      ...
      const { data } = await api.get(TRANSLADOS_EP, { params });
  ...
  const filtrados = React.useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      return nd.includes(q) || dt.includes(q) || sep.toLowerCase().includes(q) || dest.includes(q);
    });
          ...
          <div className="bg-[#f0f8ea] rounded-xl p-4 shadow">
            <div className="mb-4">
              <input
                ...
                placeholder="Buscar por nº doc., data, sepultado ou destino…"
                value={busca}
