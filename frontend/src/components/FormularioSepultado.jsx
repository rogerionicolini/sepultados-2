...
const CT_SEPULTADO = "cadastros.sepultado"; // ajuste p/ seu app_label.model
...
function TumuloDropdown({ value, onChange, api, cemiterioId, error }) {
      ...
      <form onSubmit={enviar} className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="md:col-span-2">
          <input
            ...
            className={`w-full border rounded-lg px-3 py-2 bg-white ${disabled ? "border-gray-300 opacity-60" : "border-[#bcd2a7]"}`}
          />
          {disabled && (
...
export default function FormularioSepultado() {
  ...
  async function onSubmit(e) {
    ...
    try {
      setSalvando(true);
      if (isEdit) {
        await api.put(`sepultados/${id}/`, fd, { headers: { "Content-Type": "multipart/form-data" } });
      } else {
        await api.post("sepultados/", fd, { headers: { "Content-Type": "multipart/form-data" } });
      }
      navigate("/sepultados"); // fecha e volta pra lista
    } catch (err) {
    ...
    return (
      <div key={name}>
        ...
        <input
          ...
          value={form[name] || ""}
          onChange={onChange}
          required={name === "nome"} // único obrigatório real no modelo
